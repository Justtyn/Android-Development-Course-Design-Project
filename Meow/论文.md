...
- 猫咪档案：以列表展示多只猫的基本信息（名称、品种、年龄、简介、图片），支持点击反馈，为扩展详情页预留。
- 云吸猫相册：使用网格布局浏览本地猫咪图片，展示标题/描述，保证离线浏览体验。
- 喵音 FM：展示本地音频列表，支持单曲播放/暂停、播放状态高亮、播放完成自动复位；提供进度条与快进/快退。
...
- 音频：`app/src/main/res/raw/`（`fm_*.m4a`）。
...
数据库：`meow.db`，版本号：`1`，表：`user`（`MeowDbHelper.TABLE_USER`）。
...
数据库升级策略：当前采用“删表重建”（`onUpgrade()` 中 `DROP TABLE IF EXISTS user`），适合课程演示；若后续迭代可改为增量迁移。
...
实现要点：
- Activity 本地构造猫咪档案数据，适用于无后端的课程设计。
- Adapter 负责数据绑定与点击回调；当前点击反馈采用 Toast，后续可扩展为“详情页”或“收藏”等功能。
...
页面：`activity_cat_fm.xml`、列表项 `item_fm_track.xml`；逻辑：`CatFmActivity` + `CatFmAdapter`；模型：`FmTrack`；资源：`res/raw/fm_*.m4a`。
