...
- Android Studio：Koala / Ladybug（与 AGP 8.13.1 兼容，本项目按该版本系列完成）。
- Android Gradle Plugin（AGP）：8.13.1。
- Gradle：8.13（Wrapper，`Meow/gradle/wrapper/gradle-wrapper.properties`）。
- JDK：17（Android Studio Embedded JDK）。
- Android SDK：`compileSdk` 36，`targetSdk` 36，`minSdk` 28。
- 第三方/AndroidX：AppCompat 1.6.1；Material 1.13.0；Activity 1.8.0；ConstraintLayout 2.1.4；RecyclerView 1.4.0；测试依赖 JUnit 4.13.2 / AndroidX Test Ext JUnit 1.1.5 / Espresso 3.5.1。
...
界面设计要点（与布局文件一致）：
- 登录/注册：使用 `MaterialCardView` 承载表单，`TextInputLayout` 采用 Outlined 样式，主/次按钮使用 `MaterialButton`。
- 首页：顶部 `MaterialToolbar` + 背景图，主体为卡片式 2×2 导航入口与打卡区域，整体使用柔和配色与圆角卡片统一风格。
- 列表/网格：档案与 FM 采用 `RecyclerView` 列表 + 搜索框，相册采用 `RecyclerView` 网格（2 列）突出图片浏览。
